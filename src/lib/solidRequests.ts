...
export async function parseToN3(
  ...
  return new Promise((resolve, reject) => {
    ...
    parser.parse(text, (error: Error, quad: typeof Quad, prefixes: typeof Prefixes) => {
 ...
 * @returns boolean, which indicates a correct or false input
 * 
 * This function validates the user input with a regular expression. It checks if the input is given 
 * in the right format YYYY-MM
 * 
 */
export function validateInput(input: string): Boolean {
  // REGEX: "/\d{4}-(0[1-9]|1[0-2])/"
  let date_regex = /\d{4}-(0[1-9]|1[0-2])/;
  return date_regex.test(input);
}
/**
 * 
 * @param input 
 * @returns boolean, which indicates a correct or flase input
 * 
 * This function checks if the given input field is empty (contains an empty string)
 */
 ...
 * 
 * @param solidAdress 
 * @returns URI of the current user
 * 
